...
.forEach(function({ file, flags, out }) {
    ...
    pbjs.main([
        ...
        fs.writeFileSync(out, output.replace(/"protobufjs-goe\/minimal"/g, JSON.stringify(pathToProtobufjs)));
...
.forEach(function({ file, ext }) {
    ...
    pbts.main(args.concat(file), function(err, output) {
        ...
        output = output.replace(/"protobufjs-goe"/g, JSON.stringify(pathToProtobufjs));
